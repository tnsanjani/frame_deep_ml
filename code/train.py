...
def main():
    ...
    train_dataset = StereoEventDataset(video_data_dir="/data/venkateswara_lab/frame_interpollation/data",frame_height=375,frame_width=375)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            right_events = batch['right']['events']
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                first_left_rgb = left_data['pixel_values'][:, 0]  
                last_left_rgb  = left_data['pixel_values'][:, -1] 
                encoder_hidden_states = compute_image_embeddings(first_left_rgb)
                encoder_hidden_states_ref = compute_image_embeddings(last_left_rgb)

                left_data['pixel_values'] = left_data['pixel_values'].to(accelerator.device, dtype=weight_dtype)
                right_events = right_events.to(accelerator.device, dtype=weight_dtype)

                evs_latents = right_events.to(accelerator.device, dtype=weight_dtype)
                # print(f' eve_latenmst shape is {evs_latents.shape}')
                noise = torch.randn_like(first_left_rgb).to(accelerator.device, dtype=weight_dtype)
                first_left_rgb_noisy = (first_left_rgb + noise_aug_strength * noise).to(accelerator.device, dtype=weight_dtype)
                conditions_latent = vae.encode(first_left_rgb_noisy).latent_dist.mode()
                conditions_latent = conditions_latent.unsqueeze(1).repeat(1, args.num_frames, 1, 1, 1)

                noise_ref = torch.randn_like(last_left_rgb).to(accelerator.device, dtype=weight_dtype)
                last_left_rgb_noisy = (last_left_rgb + noise_aug_strength * noise_ref).to(accelerator.device, dtype=weight_dtype)
                ...
                latents = rearrange(latents, "(b f) c h w -> b f c h w", f=num_target_frames)


                # print(f"conditions_latent shape: {conditions_latent.shape}")
                # print(f"conditions_latent_ref shape: {conditions_latent_ref.shape}")
