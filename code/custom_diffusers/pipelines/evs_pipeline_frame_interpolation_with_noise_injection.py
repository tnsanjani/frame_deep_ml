...
class EVSFrameInterpolationWithNoiseInjectionPipeline(DiffusionPipeline):
    ...
    def _encode_vae_image(
        ...
        # Force float32 precision for VAE encoding to avoid dtype mismatch
        #with torch.cuda.amp.autocast(enabled=False):
        image_latents = self.vae.encode(image.to(dtype=torch.float32)).latent_dist.mode()
    ...
    def __call__(
        ...
        image2 = self.image_processor.preprocess(image2, height=height, width=width).to(device)
                ...
                # --- Ensure generator and tensors are on the same device ---
        if generator is None:
            generator = torch.Generator(device=image1.device)
        else:
            gen_device = generator.device if hasattr(generator, "device") else None
            if gen_device != image1.device:
...
# -----------------------------------------------------------
        ...
        print("evsshape:",evs.shape,evs_latents.shape,evs_values.shape,image2.shape)
        print(evs_latents.shape,image2_latents.shape)
