...
from diffusers.utils import export_to_video as tensor2vid
...
class EVSFrameInterpolationWithNoiseInjectionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.lef_model = LatentEventFusion(in_channels=4, out_channels=4)


        # lef_path = "/mnt/workspace/zhangziran/DiffEVS/svd_keyframe_interpolation-main/ckpt/checkpoints_z0213_wfilter_v8_colorfuse_newfusemodel/svd_reverse_motion_with_attnflip/checkpoint-25000/lef_model_checkpoint.pt"
        # state_dict = torch.load(lef_path, map_location='cpu')
        # new_state_dict = {}
        # for k, v in state_dict.items():
        #     new_key = k[7:] if k.startswith('module.') else k  # Remove 'module.' prefix
        #     new_state_dict[new_key] = v
        # self.lef_model.load_state_dict(new_state_dict)
    ...
    def decode_latents(self, latents: torch.FloatTensor, num_frames: int, decode_chunk_size: int = 14):
        ...
        print("decode_chunk_size",decode_chunk_size)
    ...
    def do_classifier_free_guidance(self):
        if isinstance(self.guidance_scale, (int, float)):
            return False #self.guidance_scale > 1
        return False #self.guidance_scale.max() > 1
    ...
    def prepare_latent_feature(self, conditions_latent, evs_latents, conditions_latent_ref):
        ...
        model = self.lef_model
        model.to(conditions_latent.device)
        
        new_condition = model(conditions_latent.float(), evs_latents.float(), conditions_latent_ref.float())
        new_condition = new_condition.to(data_dtype)
    ...
    def __call__(
        ...
        evs_latents =  evs.to(device)
        if self.do_classifier_free_guidance:
            negative_evs_latents = torch.zeros_like(evs_latents)

            # For classifier free guidance, we need to do two forward passes.
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            evs_latents = torch.cat([negative_evs_latents, evs_latents])
        ...
        decode_latents = image2_latents
        latents = decode_latents
        # print(latents)
        if self.do_classifier_free_guidance:
            noise_pred_uncond2, noise_pred_cond2 = latents.chunk(2)
            # latents = noise_pred_cond2 #
            latents = noise_pred_uncond2 + self.guidance_scale * (noise_pred_cond2 - noise_pred_uncond2)
